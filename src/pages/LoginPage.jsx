...
const LoginPage = () => {
    ...
    const {login, signupWithGoogle, isLoading} = useAuthStore();
  ...
  return (
    <div className='w-screen h-screen flex justify-center items-center bg-[#A9B5DF]'>
      <div className='px-5 py-5 w-[350px] bg-white rounded-xl shadow-xl'>
        <h1 className='text-center font-semibold mb-2 text-3xl'>Welcome Back To <span className='text-[#7886C7]'>Aidspot</span></h1>
        ...
        <form onSubmit={handleSubmit} className='flex flex-col gap-3'>
          ...
          <button type='submit' disabled={isLoading} className='rounded-lg w-full py-2 bg-[#A9B5DF] hover:bg-[#7886C7] text-sm font-semibold hover:scale-105 transition-all flex items-center justify-center'>{isLoading? <Loader2 className='animate-spin size-6'/>:"Log In"}</button>
        </form>
        <p className='text-center mt-3'>Does not have an ? <Link to={'/signup'} className='hover:underline'>Account</Link></p>
